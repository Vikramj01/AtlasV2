...
export interface ScheduleRow extends ScheduleScanInputs {
  ...
  /** Set alongside last_audit_score by updateScheduleScore() — lets the regression comparator skip a false alert when a Check Register release (rule added/removed/re-weighted) moved the denominator between the two compared runs, not the site itself. See queue/regressionComparability.ts. */
  last_audit_register_version?: string | null;
  created_at: string;
  updated_at: string;
}
