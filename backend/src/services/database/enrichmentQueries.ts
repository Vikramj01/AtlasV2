...
export async function upsertSignalEnrichmentConfig(
  ...
  const payload = {
    ...
    enabled_for_tiktok: req.enabled_for_tiktok,
    enabled_for_openai: req.enabled_for_openai,
...
export function dbRowToSignalEnrichmentConfig(row: Record<string, unknown>): SignalEnrichmentConfig {
  return {
    ...
    enabled_for_tiktok: Boolean(row.enabled_for_tiktok),
    enabled_for_openai: Boolean(row.enabled_for_openai),
