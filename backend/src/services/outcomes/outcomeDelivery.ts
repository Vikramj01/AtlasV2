...
import { listDeliveredEventNamesForRecord } from '@/services/database/outcomeQueries';
...
function aggregateStatus(perDestination: Record<string, PerDestinationOutcome>): OutcomeDeliveryStatus {
...
export async function writeBackAttribution(
  ...
  try {
    const priorNames = await listDeliveredEventNamesForRecord(input.config_id, input.source_record_id);
    ...
    await provider.writeAttribution(tokens, input.tracked_object, input.source_record_id, properties);
  } catch (err) {
    logger.warn(
      { err: err instanceof Error ? err.message : String(err), configId: input.config_id, sourceRecordId: input.source_record_id },
      'Outcome attribution write-back failed — non-fatal, the delivery it describes already succeeded',
    );
  }
}
...
export async function handleLostDeal(
  ...
  stageMappings: OutcomeStageMapping[],
