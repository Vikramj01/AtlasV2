...
import type { OutcomeStageMapping, OutcomeSourceConfig, OutcomeEventForDerivedCalc } from '@/types/outcomes';

const WINDOW_START = new Date('2026-03-01T00:00:00Z');
const WINDOW_END = new Date('2026-08-28T00:00:00Z');
const CONFIG_ID = 'config-1';

function makeMapping(overrides: Partial<Pick<OutcomeStageMapping, 'id' | 'crm_stage_id' | 'is_terminal_won'>> = {}) {
  return { id: 'm-mql', crm_stage_id: 'mql', is_terminal_won: false, ...overrides };
}

function makeWonMapping(overrides: Partial<Pick<OutcomeStageMapping, 'id' | 'crm_stage_id' | 'is_terminal_won'>> = {}) {
...
function buildEvents(
  ...
  for (let i = 0; i < sampleSize; i++) {
    ...
    events.push({
      ...
      source_stage_id: stageCrmStageId,
      mapping_id: 'm-mql',
...
describe('computeStageSnapshots — structural correctness', () => {
  ...
  it('counts a record once per stage even if multiple events reference the same (record, stage) pair', () => {
    const events: OutcomeEventForDerivedCalc[] = [
      { source_record_id: 'rec-1', source_stage_id: 'mql', mapping_id: 'm-mql', conversion_value: null, currency: null, stage_changed_at: '2026-06-01T00:00:00Z' },
      { source_record_id: 'rec-1', source_stage_id: 'mql', mapping_id: 'm-mql', conversion_value: null, currency: null, stage_changed_at: '2026-06-01T00:00:01Z' },
    ];
...
describe('computeDerivedValuesForConfig', () => {
  ...
  it('computes and persists a snapshot per non-terminal-won stage', async () => {
    vi.mocked(getOutcomeSourceConfigByIdInternal).mockResolvedValue(makeConfig());
    vi.mocked(listOutcomeStageMappings).mockResolvedValue([makeMapping() as OutcomeStageMapping, makeWonMapping() as OutcomeStageMapping]);
  ...
  it('passes the default 180-day window to the events query when no override is given', async () => {
    vi.mocked(getOutcomeSourceConfigByIdInternal).mockResolvedValue(makeConfig());
    vi.mocked(listOutcomeStageMappings).mockResolvedValue([makeMapping() as OutcomeStageMapping]);
