 ...
 *
 * Covers computeOutcomeSyncHealthSignals()'s data-gathering: scoping to
...
vi.mock('@/services/database/outcomeQueries', () => ({
  listOutcomeSourceConfigsForOrg: vi.fn(),
...
import { computeOutcomeSyncHealthSignals } from '../syncHealthCheck';
import type { OutcomeSourceConfig, OutcomeStageMapping, OutcomeDerivedValueSnapshot } from '@/types/outcomes';
...
function makeSnapshot(overrides: Partial<OutcomeDerivedValueSnapshot> = {}): OutcomeDerivedValueSnapshot {
  return {
    id: 's1',
    organization_id: 'org-1',
    config_id: 'config-1',
...
describe('computeOutcomeSyncHealthSignals', () => {
  it('returns null when the org has no enabled CRM sync config', async () => {
    vi.mocked(listOutcomeSourceConfigsForOrg).mockResolvedValue([makeConfig({ sync_enabled: false })]);
    const result = await computeOutcomeSyncHealthSignals('org-1', false);
    expect(result).toBeNull();
  });

  it('scopes entirely to sync_enabled configs — a disabled config never contributes its failures', async () => {
    vi.mocked(listOutcomeSourceConfigsForOrg).mockResolvedValue([
  ...
  it('reports tokenExpired true when the connection status is expired or revoked', async () => {
    vi.mocked(listOutcomeSourceConfigsForOrg).mockResolvedValue([makeConfig({ connection_id: 'conn-1' })]);
  ...
  it('computes unresolvedIdentityRate7d and skippedWindowRate7d from recent outcome events', async () => {
    vi.mocked(listOutcomeSourceConfigsForOrg).mockResolvedValue([makeConfig()]);
  ...
  it('detects a withheld bidding-primary stage in a DERIVED-mode config', async () => {
    vi.mocked(listOutcomeSourceConfigsForOrg).mockResolvedValue([makeConfig({ value_mode: 'DERIVED' })]);
    mockDbCalls([], []);
    vi.mocked(listOutcomeStageMappings).mockResolvedValue([
  ...
  it('never checks derived stages at all for a DECLARED-mode config', async () => {
    vi.mocked(listOutcomeSourceConfigsForOrg).mockResolvedValue([makeConfig({ value_mode: 'DECLARED' })]);
    mockDbCalls([], []);

    await computeOutcomeSyncHealthSignals('org-1', false);
  ...
  it('passes existingAlertActive straight through', async () => {
    ...
    const result = await computeOutcomeSyncHealthSignals('org-1', true);
