/**
 * crmSyncHealthCheck — gathers the raw signals evaluateCrmSyncAlert()
 * (dqmAlertEvaluator.ts) needs, org-wide across every enabled
 * crm_sync_configs row. docs/prd/crm-outcome-integration.md §10, Sprint 8.
 *
...
import type { CrmSyncAlertInput } from '@/services/dqm/dqmAlertEvaluator';
...
async function countRecentOutcomeEvents(configIds: string[], sinceISO: string): Promise<OutcomeEventCounts> {
  ...
  if (error) throw new Error(`crmSyncHealthCheck: failed to read recent outcome events: ${error.message}`);
...
async function hasExpiredCrmConnection(connectionIds: string[]): Promise<boolean> {
  ...
  if (error) throw new Error(`crmSyncHealthCheck: failed to check connection status: ${error.message}`);
...
export async function computeCrmSyncHealthSignals(
  orgId: string,
  existingAlertActive: boolean,
): Promise<CrmSyncAlertInput | null> {
  const allConfigs = await listCrmSyncConfigsForOrg(orgId);
  ...
  const [tokenExpired, eventCounts, derivedWithheld] = await Promise.all([
    hasExpiredCrmConnection(connectionIds),
