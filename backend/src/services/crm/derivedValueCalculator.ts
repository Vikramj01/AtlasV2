...
import type { CrmStageMapping, NewDerivedValueSnapshotInput, OutcomeEventForDerivedCalc, CrmDerivedConfidence } from '@/types/crm';
...
function deriveConfidence(sampleSize: number, reachedWonCount: number): CrmDerivedConfidence {
...
export function computeStageSnapshots(
  ...
  stageMappings: Pick<CrmStageMapping, 'id' | 'crm_stage_id' | 'is_terminal_won'>[],
  ...
  for (const e of events) {
    if (!stagesByRecord.has(e.crm_record_id)) stagesByRecord.set(e.crm_record_id, new Set());
    stagesByRecord.get(e.crm_record_id)!.add(e.crm_stage_id);

    if (e.mapping_id && wonMappingIds.has(e.mapping_id)) {
      wonRecordIds.add(e.crm_record_id);
  ...
  return nonTerminalWonMappings.map((mapping) => {
    ...
    const confidence: CrmDerivedConfidence = hasWonAmountData
...
export async function computeDerivedValuesForConfig(
  ...
  const stageMappings = await listCrmStageMappings(configId);
