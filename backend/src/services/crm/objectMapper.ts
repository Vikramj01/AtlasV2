...
import type { CrmPipeline } from './providers/types';
import type { StageMappingInput } from '@/types/crm';
...
export function resolveStageMapping<T extends { crm_stage_id: string; enabled: boolean }>(
  ...
  return mappings.find((m) => m.enabled && m.crm_stage_id === crmStageId) ?? null;
}
