...
export const ACTION_PRIMITIVES: ActionPrimitive[] = [
  {
    ...
    optional_params: [
      ...
      { key: 'new_customer', label: 'New Customer', type: 'boolean', description: "Whether this is the customer's first purchase, sourced from your backend's order history. Feeds Google Ads' and GA4's new-customer bidding signals — leave unset if you don't have this data (Atlas can estimate it from a first-party cookie instead, see the Journey Builder setup step).", example: 'true' },
    ],
    platform_mappings: [
      { platform: 'ga4', event_name: 'purchase', param_mapping: { transaction_id: 'transaction_id', value: 'value', currency: 'currency', items: 'items', tax: 'tax', shipping: 'shipping', coupon: 'coupon', new_customer: 'new_customer' }, payload_template: 'ga4_purchase', additional_params: {} },
      ...
      { platform: 'google_ads', event_name: 'conversion', param_mapping: { transaction_id: 'transaction_id', value: 'value', currency: 'currency', new_customer: 'new_customer' }, payload_template: 'gads_conversion', additional_params: { send_to: '{{GOOGLE_ADS_CONVERSION_ID}}/{{CONVERSION_LABEL}}' } },
