      ...
      <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3">
        {BUSINESS_TYPE_OPTIONS.map((option) => (
          <button
            ...
            className={cn(
              'flex flex-col items-center rounded-xl border-2 p-5 text-left transition-all hover:shadow-sm',
              businessType === option.value
                ? 'border-[#1B2A4A] bg-[#EEF1F7] shadow-sm'
                : 'border-[#E5E7EB] bg-white hover:border-[#1B2A4A]/40'
            )}
          >
            ...
            {option.stageCount > 0 && (
              <span className="mt-3 text-xs text-[#1B2A4A]">
        ...
        <div className="mt-8">
          ...
          <div className="space-y-2">
            {templates.map((template) => (
              <div
                key={template.id}
                className="flex items-center justify-between rounded-xl border border-[#E5E7EB] bg-white px-4 py-3 hover:border-[#1B2A4A]/40 transition-colors"
              >
