/**
 * OutcomesTab — CRM Outcome Integration Sprint 8 (docs/prd/crm-outcome-integration.md §10).
 *
 ...
 * outcomesApi.listConfigs() rather than receiving a client_id/config_id prop
...
import type { OutcomeSourceConfig, OutcomeEvent, OutcomeDailyCount, OutcomeDeliveryStatus, OutcomeIdentityMethod } from '@/types/outcomes';

const PAGE_SIZE = 25;

const DELIVERY_STATUS_BADGE: Record<OutcomeDeliveryStatus, string> = {
...
const DELIVERY_STATUS_LABEL: Record<OutcomeDeliveryStatus, string> = {
  delivered: 'Delivered',
  partial: 'Partial',
  pending: 'Pending',
  failed: 'Failed',
...
const IDENTITY_METHOD_LABEL: Record<OutcomeIdentityMethod, string> = {
  click_id: 'Click ID',
...
function DeliveryStatusBadge({ status }: { status: OutcomeDeliveryStatus }) {
  return (
...
function DailyOutcomesChart({ counts }: { counts: OutcomeDailyCount[] }) {
  if (counts.length === 0) {
...
export function OutcomesTab() {
  const [configs, setConfigs] = useState<OutcomeSourceConfig[]>([]);
  const [selectedConfigId, setSelectedConfigId] = useState<string | null>(null);
  const [loadingConfigs, setLoadingConfigs] = useState(true);
  const [configsError, setConfigsError] = useState<string | null>(null);

  const [dailyCounts, setDailyCounts] = useState<OutcomeDailyCount[]>([]);
  const [loadingDaily, setLoadingDaily] = useState(false);

  const [rows, setRows] = useState<OutcomeEvent[]>([]);
    ...
    <div className="space-y-4">
      {configs.length > 1 && (
        <Select value={selectedConfigId ?? undefined} onValueChange={setSelectedConfigId}>
          ...
          <SelectContent>
            {configs.map((c) => (
              <SelectItem key={c.id} value={c.id}>{c.source_type} — {c.client_id}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      )}
      ...
      <Card className="border-console-border bg-console-surface">
        ...
        <CardContent>
          {rowsError ? (
            ...
            <p className="text-sm text-console-fg-muted">
              No outcome events yet for {selectedConfig?.source_type ?? 'this config'} — the ladder hasn't
                  ...
                  <TableBody>
                    {rows.map((r) => (
                      <TableRow key={r.id}>
                        ...
                        <TableCell className="text-sm text-console-fg whitespace-nowrap">{r.source_record_id}</TableCell>
