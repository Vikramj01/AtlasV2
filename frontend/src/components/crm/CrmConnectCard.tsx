// CrmConnectCard — two-phase OAuth connect flow (HubSpot or Salesforce),
...
// route (App.tsx), and CrmIntegrationPage renders one CrmConnectCard per
// provider on the same /crm page — this card only attempts to resolve
...
interface CrmConnectCardProps {
  ...
  onConnected?: (connectionId: string, account: CrmAccountInfo, pipelines: CrmPipeline[], provider: CrmProviderName) => void;
}

const PROVIDER_LABEL: Record<CrmProviderName, string> = { hubspot: 'HubSpot', salesforce: 'Salesforce' };
const PROVIDER_NOUN: Record<CrmProviderName, string> = { hubspot: 'portal', salesforce: 'org' };

export function CrmConnectCard({ provider, onConnected }: CrmConnectCardProps) {
  ...
  useEffect(() => {
    ...
    const discover = provider === 'hubspot' ? crmApi.discoverHubSpotPortal : crmApi.discoverSalesforceOrg;
    discover(code, state)
  ...
  async function handleConnect() {
    ...
    try {
      const { auth_url } = provider === 'hubspot'
        ? await crmApi.connectHubSpot()
        : await crmApi.connectSalesforce(undefined, sandbox);
  ...
  async function handleFinalize() {
    ...
    try {
      const finalize = provider === 'hubspot' ? crmApi.finalizeHubSpotConnection : crmApi.finalizeSalesforceConnection;
