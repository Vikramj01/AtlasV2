...
import type { CrmSyncConfig, CrmDerivedValueSnapshot, DerivedConfidence } from '@/types/crm';
...
export function DerivedValuePanel({ config }: DerivedValuePanelProps) {
  const { derivedValues, loading, errors, loadDerivedValues } = useCrmStore();

  const loadKey = `derived-values-${config.id}`;
  const loadingValues = loading[loadKey] ?? false;
  const loadError = errors[loadKey];
  const snapshots: CrmDerivedValueSnapshot[] = derivedValues[config.id] ?? [];
