...
## 12. Implementation notes (added post-build, Phases 2–3)

Recorded here rather than editing §5 above, so this document still reads as the plan that was approved — the deviation and why matters more going forward than pretending it didn't happen.

- **`fetchChangedRecords` was NOT changed to yield `OutcomeRecord`.** §5.2's literal text asks for this, but tracing the actual call site (`syncOrchestrator.ts`) found it would silently change tested behaviour: `records_processed` currently counts every record the provider yields, including one with a null `stage_id` (`syncOrchestrator.test.ts`'s "skips a record with no current stage_id without crashing" locks this in at `records_processed: 1`). `OutcomeRecord.source_stage_id` is a required, non-nullable string — satisfying that shape would require filtering no-stage records inside the provider before they're ever counted, silently changing that metric. Raw per-config `identity_property_map` resolution (`resolveIdentity()`, called today with the config's own map) and CRM-amount extraction (`extractObservedAmount()`) both also read `CrmRecord.properties`, a generic bag `OutcomeRecord` deliberately has no equivalent of. Moving all of this into each provider's `fetchChangedRecords()` was judged higher-risk than the phase justified for zero functional benefit today (nothing consumes an `OutcomeRecord` from a pull source yet). `OutcomeSource.fetchChangedRecords()` therefore still yields `CrmRecord`, documented in `sources/types.ts`'s own docstring; the outcome contract module (`contract.ts`) is built, exported, and tested, ready for Phase 3's webhook to be its first real consumer. Acceptance criterion 12 ("stamped on every ingested record") is therefore only half-true today: `OUTCOME_CONTRACT_VERSION` is exported, but nothing is ingested through the contract until Phase 3 exists to do it.
- **The push/pull `transport` discriminator lives on `OutcomeSource` itself** (`readonly transport: 'pull' | 'push'`), not on `outcome_source_configs` — every implementation of a given source type shares one transport, so a per-instance column would just duplicate what the source's own registration already states. `syncOrchestrator.ts`'s `runSync()` checks it and returns `'disabled'` for anything not `'pull'`, unreachable today (both HubSpot and Salesforce are `'pull'`) but real, tested infrastructure so Phase 3's webhook is never accidentally scheduled onto `outcomeSyncQueue`.
- **`testConnection`/`listPipelines`/`listProperties` became optional on the interface** per §5.2's instruction, which meant every `getProvider()`-typed call site needed either a runtime guard (`readinessCheck.ts`, two routes in `outcomes.ts`) or, for the two files that import `hubspotClient`/`salesforceClient` directly, keeping their exported `const`'s inferred type narrow via `satisfies OutcomeSource` instead of an explicit `: OutcomeSource` annotation — the latter would have widened those two specific, always-fully-implemented objects to the same optional-method type as a hypothetical future push source, forcing unnecessary guards at call sites that only ever see a real HubSpot/Salesforce object.
- **`CrmAccountInfo`/`CrmPipeline`/`CrmPipelineStage`/`CrmProperty`/`CrmRecord` were left unrenamed**, on the frontend and backend both. §5.2 only names `CrmProvider`→`OutcomeSource` and `CrmProviderName`→`OutcomeSourceType` explicitly; these five remain legitimately CRM-shaped concepts backing the now-optional discovery methods a non-CRM source wouldn't implement at all, not universal-layer types that were misnamed.

**Phase 3 notes:**

- **Delivery-gate design: auto-disable, not block-at-creation** — an explicit product decision made before implementation, not a deviation discovered during it. §6.3's own text ("block the client-facing consequence, not the operator's view") left open whether a brand-new source should be prevented from enabling delivery until it proves itself, or allowed to start immediately and be auto-disabled once live data shows a bad tier-3 rate. The latter was chosen: a source with zero history has no tier-3 rate to gate on in the first place (`MIN_SAMPLE_FOR_GATE`, `deliveryGate.ts`, currently 10 — below that, `evaluateDeliveryGate()` never disables, however bad the observed rate), so a block-at-creation rule would either gate on nothing (meaningless) or force an arbitrary waiting period unrelated to actual data quality. The gate instead runs inline, synchronously, right after every webhook ingest that has delivery enabled (`webhookIngest.ts`'s final block) — recomputing the tier-3 rate over the config's last 200 `outcome_events` rows (`getRecentIdentityMethodsForConfig`) and flipping `delivery_enabled` to `false` with a human-readable `delivery_disabled_reason` the moment the rate exceeds `TIER3_RATE_THRESHOLD_PERCENT` (30%, matching `dqmAlertEvaluator.ts`'s existing `UNRESOLVED_IDENTITY_HIGH_THRESHOLD` precedent rather than inventing a second unrelated threshold for the same underlying signal). Turning delivery back on is a manual, deliberate operator action (`PATCH /configs/:id` with `delivery_enabled: true`, which the route always pairs with clearing `delivery_disabled_reason` back to `null` in the same patch) — the gate never re-enables itself.
- **`eventId.ts` extracted as its own dependency-free module**, rather than webhookIngest.ts importing `computeEventId` from `syncOrchestrator.ts` as first built. `syncOrchestrator.ts` pulls in `outcomeDelivery.ts`'s full transitive dependency tree (`tokenManager`, `capiQueries`, every CAPI delivery module) purely to reach one pure SHA-256 hash function; a webhook-focused test file would have needed to mock all of that for no reason. Both `syncOrchestrator.ts` (pull) and `webhookIngest.ts` (push) now import `computeEventId` from the same tiny `eventId.ts`, so the deterministic idempotency key (§9.2) still means exactly the same thing regardless of transport.
- **`record.source_object` (contract, free-form per §5.1) is never persisted to `outcome_events.source_object`.** That column's DB CHECK still only allows `'contact'`/`'deal'` (unchanged since Phase 1) — a webhook sender declaring something like `'spreadsheet_row'` would violate it. The persisted row always uses the config's own `tracked_object` instead, matching how the pull-sync path already derives that column implicitly from the provider's own object type rather than from anything CRM-record-specific.
