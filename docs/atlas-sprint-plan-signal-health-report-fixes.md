...
## Sprint 2 — Evidence-consistency investigation + fix (Issue 2)

**Priority:** P0
**Depends on:** none

**Scope**
- **Investigation step (do first, before writing any fix):** for the openart.ai audit ID (or a fixture reproducing it), trace `STORAGE_LIFETIME_MEETS_ATTRIBUTION_WINDOW`'s evidence through both consuming paths and identify where the two `ValidationResult` objects diverge. Concretely: compare (a) whatever `audit_findings`/stored report row backs the "Issues & Fixes" page render against (b) a fresh `runRegister()` call over the same stored `AuditData`, and check whether the PDF export path (`orchestrator.ts` → `pdfGenerator.ts`) re-runs validation independently of the web view's read path rather than sharing one computed result set for a given audit run.
- Once the divergent path is identified, make it read from the single stored/computed `ValidationResult[]` for that audit run — no second computation, no second data source.
- If the investigation instead finds a genuine second rule definition sharing the `STORAGE_LIFETIME_MEETS_ATTRIBUTION_WINDOW` rule_id (e.g. a not-fully-retired v1 rule module), retire or rename it so `rule_id` stays a unique key across whatever engines are active for a report.

**Acceptance criteria (from PRD §4 Issue 2 + §7)**
- Both render paths trace back to a single evidence object per rule per audit run — provable by a test that runs one audit fixture through the full report pipeline and asserts the evidence array embedded in `journey_stages` matches the evidence embedded in `issues` for every rule_id that appears in both.
- New regression fixture covering the full v2 taxonomy (PRD §7's request) — a synthetic `AuditData` exercising every applicable rule, run once through `runRegister()`, with the resulting `ValidationResult[]` fed into every report-section builder (`buildV2LayerStages`, `interpretResults`, PDF `computeRuleOverviewStats`). Assert evidence identity across sections for every multi-section rule_id. This fixture should live under `backend/src/services/reporting/__tests__/` alongside the existing `v2Pipeline.test.ts`, and should be the standing regression net for this whole PRD, not just Issue 2 — any future rule addition that reads from two different fields in two different renderers will trip it.
