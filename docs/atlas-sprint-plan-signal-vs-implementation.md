...
### Sprint 7 · P1-02 · Implementation path classification — shipped
...
**Shipped, with the four Shopify-specific values and `SERVER_SIDE` deliberately not yet emitted — a scope narrowing found while implementing, not in the original plan.** New `ImplementationPath`/`ImplementationPathClassification` types (`types/audit.ts`) and `classifyImplementationPaths()` (new `services/provenance/implementationPathClassifier.ts`) — a pure function over `AuditData.request_provenance`, reusable by both a future report section (P1-05) and a future rule, per the PRD's own acceptance criterion; no wiring into `journeySimulator.ts`/`AuditData` needed since it's not resolved-per-crawl state, just a transform callable on demand. Reliably classifies `GTM` (initiator chain includes a `gtm.js`/`gtm-msr` loader — deliberately distinct from `gtag.js`, same split L1.1/L1.5 already keep) and `DIRECT_SCRIPT` (the platform's own loader script fired it, no GTM in the chain), and never collapses a platform's multiple simultaneous paths on one page into a single verdict — each distinct (platform, page, path) gets its own row, which is exactly the multiplicity Sprint 8 (P1-06) needs to read as a duplicate-implementation signal.

**Why the Shopify values and `SERVER_SIDE` aren't emitted yet:** a cross-origin sandboxed frame (`frame_url` genuinely cross-origin to `page_url`) is real, observable evidence of *some* isolated execution context — but asserting it's specifically a Shopify Web Pixel, without this register knowing the site is even Shopify (that's P1-03, not built yet), would be exactly the category error (a confident claim the evidence doesn't support) this whole PRD exists to fix, one layer down. Classified as `UNKNOWN` instead, with evidence naming the cross-origin frame and explicitly pointing at the missing P1-03/P1-04 dependency, rather than a guessed label. `SERVER_SIDE` would need cross-referencing `SERVER_CONTAINER_ENDPOINT_CONFIGURED`'s existing heuristic (a different data source, not derivable from `RequestInitiator` alone) — deferred to whichever of P1-05/P1-06 first actually needs it, rather than plumbed speculatively now. The full `ImplementationPath` type already has all four Shopify values plus `SERVER_SIDE`, so activating them later only changes classifier logic, not every downstream consumer's type.

**Found and fixed while grounding "the platform's own loader" against real code:** `PLATFORM_MATCHER_HOSTS` (used to identify which platform a *tracking request* belongs to) is narrower than what actually counts as a platform's own *loader script* for some platforms — e.g. Meta's matcher is `'facebook.com/tr'` only, but the Pixel's real loader is `connect.facebook.net`, a different host entirely; LinkedIn's matcher differs from its own detector's host list too. Reused L1.ts's existing vetted per-platform detectors (`trackingSignals.detectMetaPixel`/`detectTikTokPixel`/`detectLinkedInInsight`/`detectMicrosoftUet`/`detectOpenAIPixel`/`detectReddit`/`detectPinterest`) instead of introducing a third, competing host list — `google_ads` has no dedicated detector (`gtag.js` is shared with GA4, not platform-specific) and falls back to `PLATFORM_MATCHER_HOSTS.google_ads` directly, which already includes the AW-specific gtag loader shape. Caught via a test that used `connect.facebook.net` as the loader URL and initially failed.
...
### Sprint 9 · P1-03 · Commerce platform + rendering-model detection, surfaced in the report — shipped
...
**Shipped.** New `CommercePlatform`/`CommercePlatformDetection` types (`types/audit.ts`, mirrored in the frontend's `types/audit.ts`) and `commerce_platform?: CommercePlatformDetection` on `AuditData`. New pure classifier `detectCommercePlatform()` (`services/detection/commercePlatformDetector.ts`) over an already-captured `CommercePlatformSignals` shape — ports `siteDetectionService.ts`'s Shopify-CDN/WooCommerce-marker/generator-meta heuristics verbatim, plus the one genuinely new case the PRD called for: `hasHeadlessFrameworkMarker && !hasShopifyGlobal` routes to `headless` with `detected_backend` set (`shopify` or `salesforce_commerce_cloud`) rather than falling through to `custom` or misreading a decoupled storefront as a bare Shopify match — exactly the PureBorn Next.js-proxying-`cdn.shopify.com` case this sprint was scoped around. Salesforce Commerce Cloud detection (`demandware`/`/on/demandware.store/` in script/link URLs) added as a genuinely new indicator beyond what `siteDetectionService.ts` itself checks, since the PRD's platform enum names it explicitly and the PureBorn evidence review flagged it as a real gap.

Wired into `journeySimulator.ts` at the landing step, alongside the existing referrer capture and outbound-link scan: a single `page.evaluate()` reads `window.Shopify`, the three headless-framework markers (`__NEXT_DATA__`/`__NUXT__`/`___gatsby`), a `[class*="woocommerce"]` marker, and `<meta name="generator">`; a new `evaluateAcrossFrames`/`collectDeep` pass captures `link[href]` values (iframe/shadow-DOM-aware, same as the existing `script[src]` capture); `scriptSrcs` reuses the already-collected `gtmScriptSrcs` rather than re-fetching. Fails open, not closed: a thrown/rejected `page.evaluate()` leaves `commercePlatform` `undefined` — never a fabricated `'custom'` verdict from no real signal, per CLAUDE.md rule 12.

"Surfaced in the report" (this sprint's title, unlike Sprints 7/8's deliberately-unwired output): added to `SiteSetupSummary` (`buildSiteSetupSummary()` in `siteSetupDetector.ts`, passthrough from `AuditData`) and rendered as a new "Commerce Platform" card/section — first in Site Setup, ahead of the GTM Container block, so a reader has this context before any checkout/confirmation-surface finding further down the report — in both the PDF (`services/export/pdfGenerator.ts`) and the web report (`SiteSetup.tsx`). Omitted entirely (no card, no page break) when the scan captured no signal, matching the section's existing "omit rather than fabricate" convention for a pre-Sprint-9 audit.

**Deliberately not emitted: `shopify_plus`.** No signal this detector has (script/link URLs, page markers, generator meta) distinguishes a Shopify Plus merchant from standard Shopify — same "reserved in the type, not guessed at" treatment Sprint 7 gave the unactivated `ImplementationPath` Shopify sub-values. Confidence stays `'high'` only when `window.Shopify` itself is observed running (theme JS actually executing); CDN/asset evidence alone (no live global, or a headless frontend proxying the backend) is `'medium'`; the generator-meta-only fallback is `'low'`, matching the confidence semantics `siteDetectionService.ts` already uses informally via its indicator strings.
