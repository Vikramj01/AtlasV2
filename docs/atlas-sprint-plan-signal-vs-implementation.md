...
**Status** · Nothing in this plan is built. P0 blocks sending the PureBorn report; P1 is scoped but not started, per the PRD's own sequencing.
...
## One product decision needed before Sprint 1

**What does "conversion surface verified" mean: any post-landing page, or the confirmation/order page specifically?** Today's `conversionSurfaceReached()` uses the broad definition; the PRD's own conversion-*event*-fires rules only make sense against the narrow one (an event fires on the confirmation page, not the product page). **Recommendation:** add a second, narrower precondition (`requires: ['confirmation_surface']`) used only by the conversion-event-fires rules, and correct L0.3's narrative to say what it actually claims ("reached a page beyond landing") rather than implying "confirmation page reached." This changes what a currently-passing rule asserts — flagged for sign-off, not assumed.
...
### Sprint 1 · P0-01 + P0-03 · Evidence-gated layer applicability, conversion-surface gating, contradiction fix
...
- **1.3 — `confirmation_surface` precondition** (per the product decision above). Add alongside `conversion_surface` in `engine.ts`'s precondition set, computed from the step specifically labeled `confirmation`/`checkout` rather than "any distinct step." Repoint `GOOGLE_ADS_CONVERSION_EVENT_FIRES`/`META_.../TIKTOK_...` (`L5.ts:60,122`) at it. Leave L0.3 on the broad definition but correct its narrative to say what it actually claims.
...
### Sprint 2 · P0-02 · Coverage arithmetic assertion + full-layer rendering
...
### Sprint 3 · P0-04 · Conditional severity for implementation-mechanism rules
...
### Sprint 4 · P0-05 + P0-06 · Business Summary ranking + failure-copy audit
...
### Sprint 5 · Regression fixture + PureBorn re-run
...
### Sprint 6 · P1-01 · Request initiator capture
