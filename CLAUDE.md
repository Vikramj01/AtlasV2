...
- **Check Register v2** (`rule_set_version: 'v2'`, `backend/src/services/validation/register/`) — a 13-layer, rule-based validation engine superseding the v1 audit engine's hardcoded rule list. 90 rules shipped across 12 layers (L0 Scope & Configuration → L10 Deduplication, plus L12 Hygiene & Integrity; L11 Reconciliation not yet shippable — see Completed Sprints). Scan Inputs (`site_type`, `declared_platforms`, `product_domain`/`checkout_domain`/`additional_properties`, `cmp`, `test_email`/`test_phone`, ...) drive `applies_to`/`platform_scope` applicability filtering and `requires` preconditions (a rule needing a page the crawl never reached is `skipped`, not `fail`). `step_coverage` tracks per-step URL provenance and whether navigation reached a page genuinely distinct from landing; `stepUrlResolver.ts` fills `url_map` gaps via sitemap/robots.txt/link-harvest/path-heuristics (SSRF-validated, never overrides a user-supplied URL); `coverage_fingerprint` suppresses false regression alerts when the set of pages a scheduled re-scan happens to visit changes between runs; `collectDeep`/`evaluateAcrossFrames` (`dataCapture.ts`) pierce iframes and open shadow DOM for the two DOM-read call sites (outbound cross-domain link scan, GTM container-ID script scan) so a hosted checkout widget's tags/links aren't structurally invisible. Full sprint-by-sprint breakdown in `docs/atlas-sprint-plan-site-eval-coverage.md`.
...
│       ├── validation/   # register/ — Check Register v2: engine.ts (runRegister, precondition
│       │                 # gating), L0.ts-L10.ts + L12.ts rule files, platformDetection.ts
│       │                 # (PLATFORM_MATCHER_HOSTS single source of truth), reporting.ts,
│       │                 # scoring.ts; also the legacy v1 engine.ts/rules/ (runAllRules)
│       ├── audit/        # journeySimulator.ts (step loop, Cross-Domain Continuity boundary
│       │                 # probe, lead_gen form fill), dataCapture.ts (collectDeep/
│       │                 # evaluateAcrossFrames — iframe + shadow DOM traversal), orchestrator.ts,
│       │                 # stepUrlResolver.ts (page discovery), stageSimulator.ts (v1)
│       └── [others]/     # browserbase/, channels/, health/, journey/, stripe/,
...
`claude/sprint-plan-execution-o4gkol`
